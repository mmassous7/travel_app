...
import 'hotel_details_screen.dart'; // Import the hotel details screen
      ...
      child: InkWell(
        onTap: () {
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  HotelDetailsScreen(destinationName: destination.name),
            ),
          ).then((_) {
            // After returning from hotel details, navigate to activities screen
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) =>
                    ActivitiesScreen(destinationName: destination.name),
              ),
            ).then((_) {
              // After returning from activities, navigate to budget screen
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => BudgetScreen(
                      estimatedBudget: destination.price +
                          200), // Example budget calculation
                ),
              );
            });
          });
